...
def test_getB_interfaces1():
    ...
    src = mpj.magnet.Cuboid(polarization=(1, 2, 3), dimension=(1, 2, 3))
...
def test_getH_interfaces1():
    ...
    mag = (22, -33, 44)
    dim = (3, 2, 3)
    src = mpj.magnet.Cuboid(polarization=mag, dimension=dim)
    src.move(np.linspace((0.1, 0.2, 0.3), (1, 2, 3), 10), start=-1)
...
def test_dataframe_ouptut():
    ...
    sources = [
        mpj.magnet.Cuboid(polarization=(0, 0, 1000), dimension=(1, 1, 1)).move(
    ...
    sens1 = mpj.Sensor(position=(0, 0, 1), pixel=pixel, style_label="sens1")
    sens2 = sens1.copy(position=(0, 0, 3), style_label="sens2")
    sens_col = mpj.Collection(sens1, sens2)
...
def test_dataframe_ouptut_sumup():
    ...
    sources = [
        mpj.magnet.Cuboid(polarization=(0, 0, 1000), dimension=(1, 1, 1)),
        mpj.magnet.Cylinder(polarization=(0, 1000, 0), dimension=(1, 1)),
    ]
    df_field = mpj.getB(sources, (0, 0, 0), sumup=True, output="dataframe")
...
def test_dataframe_ouptut_pixel_agg():
    """test pandas dataframe output when sumup is True"""
    src1 = mpj.magnet.Cuboid(polarization=(0, 0, 1000), dimension=(1, 1, 1))
    sens1 = mpj.Sensor(position=(0, 0, 1), pixel=np.zeros((4, 5, 3)))
...
def test_getB_on_missing_dimensions(module, class_, missing_arg):
    ...
    with pytest.raises(
        ...
        getattr(getattr(mpj, module), class_)().getB([0, 0, 0])
...
def test_getB_on_missing_excitations(module, class_, missing_arg, kwargs):
    ...
    with pytest.raises(
        ...
        getattr(getattr(mpj, module), class_)(**kwargs).getB([0, 0, 0])
...
def test_getHBMJ_self_consistency(field):
    ...
    F4 = getattr(mpj.Collection(src, sens), f"get{field}")()
